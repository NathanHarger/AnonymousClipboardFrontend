...
getCookie(name) {
  ...
  submit_text(){
      ...
      axios.post(process.env.REACT_APP_API_DOMAIN +'/api/clipboard/',{
  ...
  submit_file(){
    ...
    formData.append("csrfttoken", csrft);

    console.log(data_file.files[0])
    axios.post(process.env.REACT_APP_API_DOMAIN +'/api/clipboard/', formData,{  headers: {
  ...
  submit_id(){
    ...
    var metadata;
    axios.get(process.env.REACT_APP_API_DOMAIN +'/api/clipboard/getMetadata/', {params:{session_id: data_id}}) 
    .then(function (response) {
      metadata = response.data
      //console.log(response);
      //var media_type = response.data['media_type']
      //console.log(media_type)
      axios.get(process.env.REACT_APP_API_DOMAIN +'/api/clipboard/getData/', {params:{session_id: data_id}})
        .then(function(response){
          console.log(response)
